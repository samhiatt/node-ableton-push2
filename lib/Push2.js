...
class Push2 extends EventEmitter {
    ...
    return new Promise(function (resolve, reject) {
      self.midi.on('sysex',function handler(msg) {
        if (msg.bytes[4]==2) { // device identity reply
          ...
          self.emit('received_deviceId',self.deviceId);
  ...
  setMidiMode(mode){
    var bytes = [0x0A];
    var val = null;
    var modes = ['live','user','both'];
    mode = mode.toLowerCase();
    val = modes.indexOf(mode);
    if (val<0) throw new Error("Expected mode to be 'live', 'user', or 'both'.");
    bytes.push(val);
    this._sendSysexRequest(bytes).then((resp)=>{
      if (resp.bytes[7]!=val)
        throw new Error("Tried to set MIDI mode to ${mode} but responded with mode ${modes[resp.bytes[7]]}");
    });
  }
